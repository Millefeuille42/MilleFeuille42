...
inline static t_win		draw_line(t_win c, t_ray r, int x, t_draw lim)
{
	...
	while (i[0] < (c.conf->res.y - 1))
	{
		...
		if (i[0] < lim.s)
			image_pixel_put((t_ivec){x, i[0]}, c.img, c_col, c.conf->res);
		else if (i[0] < lim.e)
		{
			col = text_spot(r, *c.conf->t, i[0], lim);
			col = shade_text(col, lim.dist);
			image_pixel_put((t_ivec){x, i[0]}, c.img, col, c.conf->res);
		}
		else
			image_pixel_put((t_ivec){x, i[0]}, c.img, f_col, c.conf->res);
		i[0]++;
		i[1]--;
	}
	return (c);
