...
t_philo		philo_setup(const t_param *args, int id, pthread_mutex_t *forks)
{
	t_philo	ret;
	...
	ret.forks_taken = 0;
	set_times(&ret, args);
	ret.forks[1] = &forks[ret.id % args->n_philo];
	ret.forks[0] = &forks[ret.id - 1];
	pthread_mutex_init(&ret.death_lock, NULL);
	return (ret);
}
